...
public class ProductServiceImpl implements ProductService {
    ...
    public List<Product> queryPersonByArray(int currPage, int pageSize,
        ...
        try {
            ...
            // System.out.println("list.subList(firstIndex, lastIndex)//"+
    ...
    public Integer countPageLike(String productName, Double price, Integer opt) {
        ...
        try {
            ...
            connection.setAutoCommit(false);
            ProductDao pd = new ProductDaoImpl();
            Integer count = pd.countPageLike(productName, price, opt);
            System.out.println(" {{service count}=" + count);
            if (count % 3 == 0) {
                pageInteger = count / 3;
            } else {
                pageInteger = count / 3 + 1;
            }
            connection.commit();
            System.out.println(" {{service pageInteger}=" + pageInteger);
            return pageInteger;
    ...
    @Override
    public List<Product> queryPersonByArrayLike(int currPage, int pageSize,
                                                String productName, Double price, Integer opt, Integer countPage) {
        ...
        try {
            ...
            connection.setAutoCommit(false);
            ProductDao pd = new ProductDaoImpl();
            lists = pd.queryPersonByArrayLike(productName, price, opt);

            System.out.print("\nqueryPersonByArrayLike-countPage=" + countPage);
            System.out.print("\nqueryPersonByArrayLike-currPage=" + currPage);
            int firstIndex = (currPage - 1) * pageSize + 1;
            int lastIndex = currPage * pageSize;
            // System.out.println("list.subList(firstIndex, lastIndex)//"+
            // list.subList(firstIndex, lastIndex));
